...
- 当前目录不是 git 仓库，不能依赖 `git restore` 恢复误删或误改内容。
...
- `RefpropToCcm.spec` 和 `refprop_to_ccm_gui.py` 是以后重新打包 exe 可能会用到的文件；当前未打包源码运行不依赖它们。
- `build`、`dist`、`release` 曾按用户确认删除过；如果现在又出现，是后续构建重新生成的。
- `out` 和 `refprop_to_ccm\__pycache__` 是运行/测试生成内容，可再生，不是核心源码。
...
用户要求检查项目目录，把不需要的内容删除或放到单独文件夹，并对有用内容做本地 Git 提交。实际处理采用“移动到 `no-use`，不直接删除”的方式。
...
- `.gitignore` 已加入 `no-use/`、`out/`、`build/`、`dist/`、`release/`、`*.sim`、`*.sim~`、`*.log`、`.env` 等规则，避免生成文件、仿真文件和无关目录进入 Git。
