...
把 REFPROP 物性数据导出为 STAR-CCM+ 可用的材料参数，并生成一个 Java 宏，把数据写入指定 STAR-CCM+ 项目中已有连续体的已有 gas/liquid 相材料属性。v1.0.0测试时选择连续体模型为三维、定常、多相、两相热力学平衡、分离流体焓、湍流、K-Epsilon湍流、壁面距离、两层全y+壁面处理。气相模型选择用户自定义的EOS/多项式密度，液体选择恒密度或者用户自定义的EOS。
多项式密度
## 当前版本做什么
...
版本提示：当前写入宏和打包程序仅在 `Simcenter STAR-CCM+ 2602 Build 21.02.007 (2026.02)` 上完成实际运行与回读验证。其他 STAR-CCM+ 版本可能因 Java API、单位名称或材料属性方法差异导致写入失败，使用前应先在 `.sim` 副本上验证；程序不保证兼容未经验证的版本。

截图中的“生成热”当前按用户要求使用 REFPROP 饱和状态比焓写入：气态材料使用饱和气体焓，液态材料使用饱和液体焓。它不是严格化学标准生成焓。

STAR-CCM+ 中文界面中气体材料显示为“标准状态焓”且单位为 `J/kg-K` 的字段，Java API 实际为 `StandardStateEntropyProperty`，属于标准状态熵。程序从 REFPROP 读取饱和气体熵后写入该字段；它不能使用单位为 `J/kg` 的生成热数值。
